		...
		<div class="tbl_frm01 tbl_wrap">
			<table>
				...
				<tbody>
					...
					<tr>
						<th scope="row"><label for="it_order">핫딜 적용 사용</label></th>
						<td colspan="2">
							<input type="radio" name="it_1_subj" value="" id="it_1_1"<?php echo ($it['it_1_subj'] == '') ? ' checked' : ''; ?> class="hotdeal-use" />
							<label for="it_1_1">사용안함</label>
							<input type="radio" name="it_1_subj" value="on" id="it_1_2"<?php echo ($it['it_1_subj'] == 'on') ? ' checked' : ''; ?> class="hotdeal-use" />
							<label for="it_1_2">사용</label>
						</td>
					</tr>
					<tr id="hotdeal_timer_wrap">
						<th scope="row"><label for="it_order">핫딜 D-day</label></th>
						<td colspan="2">
							<div style="position: relative;">
								<input type="text" name="it_1" class="form-control" id="hotdeal_timer" value="<?php echo $it['it_1']; ?>" >
							</div>
						</td>
					</tr>
					<tr>
...
(function($) {
	jQuery(document).ready(function() {
		var checkjedId = jQuery('.hotdeal-use:checked').attr('id');
		if(checkjedId == 'it_1_2') {
			jQuery('#hotdeal_timer_wrap').show();
		}
		jQuery(document).on('change', '.hotdeal-use', function() {
			var id = $(this).attr('id');
			if(id == 'it_1_2') {
				jQuery('#hotdeal_timer_wrap').slideDown();
			} else {
				jQuery('#hotdeal_timer_wrap').slideUp();
			}
		});
		jQuery('#hotdeal_timer').datetimepicker({
