...
if (!$is_member) {
	...
	alert_close('Only members can send mail.');
}

// 스팸으로 인한 코드 수정 060809
//if (substr_count($to_email, "@") > 3) alert("최대 3명까지만 메일을 발송할 수 있습니다.");
if (substr_count($to_email, "@") > 1) {
	//alert('메일 주소는 하나씩만 입력해 주십시오.');
...
if ($_SESSION["ss_recommend_datetime"] >= (G5_SERVER_TIME - 120)) {
	...
	alert("You can not send mail in a very short period of time.");
}
...
if ($recommendmail_count > 3) {
	...
	alert_close('You can only send a certain number of messages after connecting once.\\n\\nPlease login or log in again to continue sending mail.');
}
...
if ($_POST["token"] && get_session("ss_token") == $_POST["token"]) {
	...
	alert_close("There was an error sending mail.");
	exit;
}
...
if (!$it['it_id']) {
	// alert("등록된 상품이 아닙니다.");
	alert("It is not a registered product.");
}
...
//alert("메일을 전달하였습니다.");
alert("I have forwarded the email.");
window.close();
</script>
