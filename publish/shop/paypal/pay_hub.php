...
$store_account  = $_POST['payer_email'];
$item_name      = $_POST['item_name'];
$currency       = $_POST['mc_currency'];

$payment_status = $_POST['payment_status'];
$od_id          = clean_xss_tags($_POST['item_number']); // 주문번호
$tno            = clean_xss_tags($_POST['txn_id']); // 응답 시 발생 거래번호
$ipn_track_id   = clean_xss_tags($_POST['ipn_track_id']); // IPN unique identifier
$pg_price       = $_POST['mc_gross']; // 결제금액
$buyer_account  = $_POST['receiver_email'];
...
$log_dir = G5_DATA_PATH.'/paypallog'; 
...
$error = "";
if (strtoupper($payment_status) == 'COMPLETED') { // 성공
    ...
    curl_setopt_array($ch,array(
        CURLOPT_URL => $pp_conf_action_url,
        CURLOPT_VERBOSE => 1,
    ...
    if (!$result) { // 검증실패
        $error = $od_id." : 전송값 검증에 실패하였습니다.";
        wz_fwrite_log($log_dir."/errorlog.log", $error);
        exit;
    }
    ...
    $row = sql_fetch($sql);
    if (!$row['od_id']) {
        $error = $od_id." : 주문정보가 존재하지 않음.";
        wz_fwrite_log($log_dir."/errorlog.log", $error);
        exit;
    }

    $od_time = $row['dt_time']; // uid 값 설정을 위한 처리.
    
    $data = unserialize(base64_decode($row['dt_data']));
    $tmp_cart_id = $data['wz_cart_id'];

    if (get_cart_count($tmp_cart_id) == 0) { // 장바구니에 담기
        $error = $od_id." : 장바구니가 비어 있음. : ". $tmp_cart_id;
        wz_fwrite_log($log_dir."/errorlog.log", $error);
        exit;
    }
    
    $od_paypal_price = $row['dt_paypal_price'];
    if ($od_paypal_price != $pg_price) { // 결제금액 일치 확인
        $error = $od_id." : 결제금액 일치하지 않음. : ".$od_paypal_price." == ".$pg_price;
        wz_fwrite_log($log_dir."/errorlog.log", $error);
        exit;
    }
    ...
    if ($i == 0) {
        $error = $od_id." : 장바구니가 비어 있습니다. 이미 주문하셨거나 장바구니에 담긴 상품이 없는 경우입니다.";
        wz_fwrite_log($log_dir."/errorlog.log", $error);
        exit;
    }


    $good_mny           = (int)$data['pl_amount'];
    ...
    $od_mobile                  = $data['wz_is_mobile'];
    $od_ip                      = $data['wz_user_ip'];
    ...
    if ((int)($row['od_price'] - $tot_cp_price) !== $i_price) {
        $error = $od_id." : Error. : ".$row['od_price']." - ".$tot_cp_price." != ".$i_price;
        wz_fwrite_log($log_dir."/errorlog.log", $error);
        exit;
    }
    ...
    if ((int)($send_cost - $tot_sc_cp_price) !== (int)($i_send_cost - $i_send_coupon)) {
        $error = $od_id." : Error.. : (".$send_cost." - ".$tot_sc_cp_price.") !== ".$i_send_cost." - ".$i_send_coupon;
        wz_fwrite_log($log_dir."/errorlog.log", $error);
        exit;
    }
    ...
    if($send_cost2 !== $i_send_cost2) {
        $error = $od_id." : Error... : ".$send_cost2." !== ".$i_send_cost2;
        wz_fwrite_log($log_dir."/errorlog.log", $error);
        exit;
    }
    ...
    if (($i_temp_point > (int)$temp_point || $i_temp_point < 0) && $config['cf_use_point']) {
        $error = $od_id." : Error.... : (".$i_temp_point." > (int)".$temp_point." || ".$i_temp_point." < 0) &amp;&amp; ".$config['cf_use_point'];
        wz_fwrite_log($log_dir."/errorlog.log", $error);
        exit;
    }
    ...
    if ($od_temp_point)
    {
        if ($member['mb_point'] < $od_temp_point) {
            $error = $od_id." : 회원님의 포인트가 부족하여 포인트로 결제 할 수 없습니다.";
            wz_fwrite_log($log_dir."/errorlog.log", $error);
            exit;
        }
    }
    ...
    {
        $od_tno             = $tno;
        $od_app_no          = $ipn_track_id;
        $od_receipt_price   = $good_mny;
        $od_receipt_point   = $i_temp_point;
        $od_receipt_time    = G5_TIME_YMDHIS;
        $od_deposit_name    = $od_name;
        $od_bank_account    = $store_account;
        $pg_price           = $good_mny;
    ...
    if($tno) {
        if((int)$order_price !== (int)$pg_price) {
            $cancel_msg = $od_id." : 결제금액 불일치 : ".$order_price." !== (int)".$pg_price;
            wz_fwrite_log($log_dir."/errorlog.log", $cancel_msg);
            exit;
        }
    }
    ...
    $sql = " insert {$g5['g5_shop_order_table']}
                set od_id             = '$od_id',
                    ...
                    od_paypal_price   = '$od_paypal_price'
                    ";
    ...
    // 주문 정보 임시 데이터 삭제
    $sql = " delete from {$g5['g5_shop_order_data_table']} where od_id = '$od_id' and dt_pg = '{$default['de_pg_service']}' ";
    sql_query($sql);
