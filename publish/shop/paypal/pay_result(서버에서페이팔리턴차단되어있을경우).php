...
$od_id          = clean_xss_tags($_GET['item_number']); // 주문번호
$tno            = clean_xss_tags($_GET['tx']); // 응답 시 발생 거래번호
$ipn_track_id   = ''; // IPN unique identifier
$pg_price       = $_GET['amt']; // 결제금액
...
wz_fwrite_log($log_dir."/query_result_".date("Ymd").".txt", $log_txt);
...
if (strtoupper($payment_status) == 'COMPLETED') { // 성공
    ...
    if (!$result) { // 검증실패
        ...
        wz_fwrite_log($log_dir."/errorlog.txt", $error);
        exit;
    }
    */
    ...
    if($tno) {
        if((int)$order_price !== (int)$pg_price) {
            ...
            wz_fwrite_log($log_dir."/errorlog.txt", $cancel_msg);
            exit;
        }
    }
