...
<div id="sod_frm">
	...
	<div class="tbl_head03 tbl_wrap od_prd_list">
		...
		for ($i=0; $row=sql_fetch_array($result); $i++)
		{
			...
			{
				case 1:
					$ct_send_cost = 'Cash on delivery';//착불
					break;
				case 2:
					$ct_send_cost = 'free';//무료
					break;
				default:
					$ct_send_cost = 'prepayment';//선불
					break;
			}
		...
		if ($i == 0) {
			...
			alert('Your shopping cart is empty.', G5_SHOP_URL.'/cart.php');//장바구니가 비어 있습니다.
		} else {
			// 배송비 계산
		...
		<section id="sod_frm_pay">
			...
			<div class="pay_tbl">
				...
				<tr>
					...
					<td>
						...
						<button type="button" id="od_coupon_btn" class="btn_frmline">Apply coupon</button>
					</td>
				</tr>
				<?php } ?>
				<?php if($sc_cnt > 0) { ?>
				<tr>
					...
					<td>
						...
						<button type="button" id="sc_coupon_btn" class="btn_frmline">Apply coupon</button>
					</td>
				</tr>
				<?php } ?>
				...
				<tr>
					...
					<td>$<strong id="od_send_cost2">0</strong><br>(It is the carriage such as the duties added according to the area.)</td><!-- 지역에 따라 추가되는 도선료 등의 배송비입니다. -->
				</tr>
				</tbody>
				</table>
			</div>
			...
			<div id="od_pay_sl">
				...
				if (!$default['de_card_point'])
					...
					echo '<p id="sod_frm_pt_alert">Points can not be earned in cases other than <strong>bank transfer.</strong></p>';
				...
				if ($default['de_bank_use']) {
					$multi_settle++;
					echo '<input type="radio" id="od_settle_bank" name="od_settle_case" value="무통장" '.$checked.'> <label for="od_settle_bank" class="lb_icon bank_icon">Bank transfer</label>'.PHP_EOL;
					$checked = '';
				}
