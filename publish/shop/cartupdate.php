...
if (!$tmp_cart_id)
{
	...
	alert('You can not proceed further.\\n\\nIt appears that you have set your browser\'s Allow Cookies to be disabled.\\n\\nPlease enable cookies in your browser\'s Internet Options.\\n\\nIf it still does not work, please contact the store operator.');
}
...
if ($member['mb_level'] < $default['de_level_sell'])
{
	...
	alert('You do not have permission to purchase the item.');
}
...
else // 장바구니에 담기
{
	...
	for($i=0; $i<$count; $i++) {
		...
		if(!$it['it_id']) {
			...
			alert('Product information does not exist.');
		}
		...
		if($it['it_buy_min_qty'] || $it['it_buy_max_qty']) {
			...
			if($it['it_buy_min_qty'] > 0 && $sum_qty < $it['it_buy_min_qty']) {
				...
				alert('Total number of '.$it['it_name'].' options Please order '.number_format($it['it_buy_min_qty']).' or more');
			}
			...
			if($it['it_buy_max_qty'] > 0 && $sum_qty > $it['it_buy_max_qty']) {
				...
				alert('Total number of '.$it['it_name'].' option options Please order less than '.number_format($it['it_buy_max_qty']).' pieces.');
			}
			...
			if($it['it_buy_max_qty'] > 0) {
				...
				if(($sum_qty + $row4['ct_sum']) > $it['it_buy_max_qty']) {
					...
					alert('Total number of '.$it['it_name'].' option options Please order less than '.number_format($it['it_buy_max_qty']).' pieces.', './cart.php');
				}
			}
		}
		...
		if($sw_direct) {
			for($k=0; $k<$opt_count; $k++) {
				...
				if ($ct_qty + $sum_qty > $it_stock_qty)
				{
					...
					alert('There is not enough stock in '.$io_value.".\\n\\Current stock quantity : " . number_format($it_stock_qty - $sum_qty) );
				}
			}
		}
		//--------------------------------------------------------
		...
		for($k=0; $k<$opt_count; $k++) {
			...
			if($row2['ct_id']) {
				...
				if ($tmp_ct_qty + $ct_qty > $tmp_it_stock_qty)
				{
					...
					alert('There is not enough stock in '.$io_value.".\\n\\Current stock quantity : " . number_format($tmp_it_stock_qty) );
				}
